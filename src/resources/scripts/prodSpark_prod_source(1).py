...
if __name__ == "__main__":
	...
	ofpriceDF1 = spark.sql("""SELECT init_date, LAG(C1.day_nav1, 1) OVER(PARTITION BY C1.fund_code  ORDER  BY  C1.init_date) as next_nav
                               FROM (SELECT C1.init_date, C1.nav as day_nav1, C1.fund_code as FP_CODE, A.FUND_NAME as FP_NAME
                                       FROM ods_ofprice C1 
									   JOIN hs08_ofstkcode A 
									 ON A.FUND_CODE = C1.FUND_CODE 
								    WHERE C1.init_date >= %d AND C1.init_date < %d AND A.FUND_STATUS = 0 
									""" % (STARTTIME, ENDTIME))
	ofpriceDF1.createOrReplaceTempView("ofpriceTmp")
	
	
	prodpriceDF = spark.sql("""SELECT C2.INIT_DATE, C2.NET_VALUE as day_nav1, C2.prod_code as FP_CODE, A1.PROD_NAME as FP_NAME
                                           FROM hs08_his_prodprice C2 
                                           JOIN hs08_prodcode A1
                                             ON A1.prod_code = C2.prod_code 
                                     WHERE  A1.PROD_STATUS = 1 and C2.init_date >= %d AND C2.init_date < %d
									 ) """ % (STARTTIME, ENDTIME,STARTTIME, ENDTIME)
	prodpriceDF.createOrReplaceTempView("prodpriceTmp")
	
	ofpriceDF = ofpriceDF1.
	
								
	
							 
	ofpriceDF = spark.sql("""SELECT init_date, LAG(C1.day_nav1, 1) OVER(PARTITION BY C1.fund_code  ORDER  BY  C1.init_date) as next_nav
                               FROM (SELECT C1.init_date, C1.nav as day_nav1, C1.fund_code as FP_CODE, A.FUND_NAME as FP_NAME
                                       FROM ods_ofprice C1 
									   JOIN hs08_ofstkcode A 
									 ON A.FUND_CODE = C1.FUND_CODE 
								    WHERE C1.init_date >= %d AND C1.init_date < %d AND A.FUND_STATUS = 0 
								  union  
								    SELECT C2.INIT_DATE, C2.NET_VALUE as day_nav1, C2.prod_code as FP_CODE, A1.PROD_NAME as FP_NAME
                                           FROM hs08_his_prodprice C2 
                                           JOIN hs08_prodcode A1
                                             ON A1.prod_code = C2.prod_code 
                                     WHERE  A1.PROD_STATUS = 1 and C2.init_date >= %d AND C2.init_date < %d
